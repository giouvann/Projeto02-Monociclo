...
module top_level (
    ...
    wire [25:0] jaddr  = instruction[25:0]; // campo address para j/jal
    ...
    wire [4:0]  WriteReg_pre;  // saída do MUX_REGDST (rt ou rd)
    wire [4:0]  WriteReg;      // saída final (WriteReg_pre ou $31, para jal)
    wire [31:0] ReadData1, ReadData2;
    wire [31:0] WriteData_pre; // saída do MUX_MEMTOREG (ALU result ou DMEM)
    wire [31:0] WriteData;     // saída final (WriteData_pre ou PCplus4, para jal)
    ...
    // Link (jal): forca WriteReg = $31 (5'b11111), independente de RegDst
    mux2x1_5bits MUX_LINK_REG (
        .A (WriteReg_pre),
        .B (5'b11111),
        .S (Link),
        .Y (WriteReg)
    ...
    // Link (jal): forca WriteData = PC+8, independente de MemtoReg
    wire [31:0] PCplus8 = PCplus4 + 32'd4;

    mux2x1_32bits MUX_LINK_DATA (
        .A (WriteData_pre),
        .B (PCplus8),
        .S (Link),
        .Y (WriteData)
    ...
    wire        PCSrc;
    wire [31:0] pc_branch_mux_out; // PCplus4 ou branch_target

    assign branch_offset = imm_ext << 2;
    assign branch_target = PCplus4 + branch_offset;

    // PCSrc = 1 quando o desvio condicional deve ser tomado
    ...
    // Endereço de destino do jump incondicional (j / jal):
    // {PCplus4[31:28], jaddr, 2'b00}
    wire [31:0] jump_target;
    assign jump_target = {PCplus4[31:28], jaddr, 2'b00};

    // Jump (j/jal): seleciona entre (PCplus4 ou branch_target) e jump_target
    wire [31:0] pc_jump_mux_out;

    mux2x1_32bits MUX_JUMP (
        .A (pc_branch_mux_out),
        .B (jump_target),
        .S (Jump),
        .Y (pc_jump_mux_out)
    ...
    // JumpReg (jr): seleciona entre o resultado anterior e ReadData1
    mux2x1_32bits MUX_JR (
        .A (pc_jump_mux_out),
        .B (ReadData1),
        .S (JumpReg),
        .Y (nextPC)
